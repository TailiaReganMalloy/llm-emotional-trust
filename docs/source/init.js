...
window.util.colors = [1 - 0.25, 0.25].map(util.color);
...
window.pairs = [
  {
    ...
    ariaLabel:
      'Scatter plot of differences in purchases between New York and Texas. Oil, cotten and land are associated more with Texas; Pictures and perfume are more associated with New York',
    alts: [
      {
  ...
  },
  {
    ...
    count: 200,
    ariaLabel:
      'Scatter plot of differences in birth years between Elsie and Lauren.',
  },
  {
    class: 'jim-jane',
    ...
    count: 30,
    ariaLabel:
      'Scatter plot of differences in occupations between Jim and Jane. Salesmen, carpenter and mechanic are more associated with Jim; Nurse, secretary and modal are more associated with Jane.',
  },
  {
    class: 'nurse-name',
    ...
    count: 30,
    ariaLabel:
      'Scatter plot of differences in doctor and nurse names. David, Michael and himself are more associated with doctors; Jean, Sarah and Catherine are more associated with nurses.',
  },
  {
    ...
    count: 30,
    ariaLabel:
      'Scatter plot of differences in doctor and nurse names in the Zari model. He and she are equally associated with both. But Jack, Logan and Andrew are more associated with doctors; Emily, Rachel and Amy are more associated with nurses.',
  },
  {
...
window.init = async function () {
  ...
  if (!window.bertLargeVocab) {
    var text = await (await fetch(assetPath('source/data/bert_large_vocab.txt'))).text();
    window.bertLargeVocab = text.split('\n');
  }

  sents.forEach((sent, i) => {
    console.log('[init.js] Calling initSent for sent', i, sent);
    initSent(sent);
  });
  sleep(10);
...
!(function () {
  console.log('[init.js] IIFE running. window.init type:', typeof window.init);
  var lastInnerWidth = null;

  function resize() {
    console.log('[init.js] resize() called. innerWidth:', window.innerWidth, 'lastInnerWidth:', lastInnerWidth);
    if (lastInnerWidth == window.innerWidth) return;
    lastInnerWidth = window.innerWidth;

    console.log('[init.js] Calling window.init() from resize...');
    window.init();
  }
  resize();
...
window.addLockedTooltip = function (sel) {
  sel.on('mouseover', function (d, i) {
    ttSel.html(d).select('.footend').remove();
