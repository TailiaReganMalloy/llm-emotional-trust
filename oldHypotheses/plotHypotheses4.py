...
OUTPUT_PATH = "plots/h4_nonweird_emotional_vs_analytical.png"
ALPHA = 0.05
...
def build_nonweird_dataframe(df: pd.DataFrame) -> pd.DataFrame:
    required = [
    ...
    nonweird_df = work[~work["weird_like"]].copy()
    pair_df = nonweird_df[["emotional_change_z", "analytical_change_z"]].dropna()
    if len(pair_df) < 2:
        raise ValueError("Not enough paired Non-WEIRD-like observations for Hypothesis 4 visualization.")
    return pair_df


def collect_h4_results(pair_df: pd.DataFrame) -> tuple[pd.DataFrame, float, float, int]:
...
def plot_h4(results: pd.DataFrame, t_stat: float, p_value: float, n: int) -> None:
    ...
    ax.set_xlabel("Trust-Type Change (Non-WEIRD-like Group)", fontsize=18)
    ax.set_ylabel("Standardized Mean Change (Post - Pre)", fontsize=18)
    ax.set_title("Hypothesis 4: Non-WEIRD-like Emotional vs Analytical Change", fontsize=28)
...
def main() -> None:
    ...
    pair_df = build_nonweird_dataframe(df)
    results, t_stat, p_value, n = collect_h4_results(pair_df)
    plot_h4(results, t_stat, p_value, n)

    print("Saved plot:", OUTPUT_PATH)
    print("\nHypothesis 4 visualization values:")
