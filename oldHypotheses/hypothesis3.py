...
Hypothesis 3: The WEIRD-like group shows no significant difference between
...
OUTPUT_PATH = Path("./plots/hypothesis3_weird_type_difference.csv")
ALPHA = 0.05
...
def analyze_weird_group(df: pd.DataFrame) -> dict[str, object]:
...
def latex_line(row: pd.Series) -> str:
	if pd.isna(row["Paired t"]):
		return (
			...
			"within the WEIRD-like group, but there were insufficient paired observations."
		)
	...
	return (
		...
		"the WEIRD-like group. Results indicated that Emotional Trust Change ($M = "
...
def main() -> None:
	...
	result = pd.DataFrame([analyze_weird_group(analysis)])

	OUTPUT_PATH.parent.mkdir(parents=True, exist_ok=True)
	result.to_csv(OUTPUT_PATH, index=False)

	print("Hypothesis 3: WEIRD-like type-comparison test")
