...
def _age_to_group(age_value: object) -> str | None:
...
age_source = "Age_demographic" if "Age_demographic" in combined.columns else "Age"
combined["Age Group"] = combined[age_source].apply(_age_to_group)
combined = combined[combined["Age Group"].notna()].copy()
...
panel_specs = [
    ("Emotional Impact Delta", "Emotional", "Below 38", 0, 0),
    ("Emotional Impact Delta", "Emotional", "38 and Above", 0, 1),
    ("Analytical Trust Impact Delta", "Analytical", "Below 38", 1, 0),
    ("Analytical Trust Impact Delta", "Analytical", "38 and Above", 1, 1),
]

all_summaries = []
fig, axes = plt.subplots(2, 2, figsize=(14, 10), sharey=True)

for impact_col, impact_label, age_label, row_i, col_i in panel_specs:
    ax = axes[row_i, col_i]
    subset = combined[combined["Age Group"] == age_label].copy()
    ...
    summary_df["age_group"] = age_label
    ...
    ax.set_title(f"{impact_label} - {age_label}")
...
fig.suptitle("Overall Trust Impact by Condition and Age Group", fontsize=14)
plt.tight_layout(rect=[0, 0, 1, 0.97])

out_dir = Path("./analysis")
out_dir.mkdir(parents=True, exist_ok=True)

plot_path = out_dir / "age_split_emotional_analytical_2x2.png"
plt.savefig(plot_path, dpi=300, bbox_inches="tight")
plt.close()

summary_all_df = pd.concat(all_summaries, ignore_index=True)
summary_path = out_dir / "age_split_emotional_analytical_summary.csv"
summary_all_df.to_csv(summary_path, index=False)

print("Age-split emotional and analytical impact summary:")
print(summary_all_df.to_string(index=False))
print(f"\nAge source column used: {age_source}")
