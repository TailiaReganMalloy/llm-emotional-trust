...
def make_group_condition_sentiment_plot(df: pd.DataFrame, primary: pd.DataFrame, output_path: Path) -> None:
    ...
    ax.set_xticklabels([group_display[g] for g in groups], fontsize=16)
    ax.set_ylabel("Mean sentiment score", fontsize=18)
    ax.set_xlabel("Participant group", fontsize=18)
    ax.set_title("Open-response sentiment by Group and Condition", fontsize=22, pad=12)
    ax.tick_params(axis="y", labelsize=15)
    ax.legend(title="Condition", fontsize=14, title_fontsize=15, loc="best")
    ...
    sig_lines = [
        ...
        (
            "Overall group (WEIRD vs non-WEIRD): "
...
def make_sentiment_scatter_panels(df: pd.DataFrame, output_path: Path) -> None:
    ...
    for ax, outcome in zip(flat_axes, outcomes):
        ...
        if len(plot_df) >= 3:
            ...
            corr_lines = [
                ...
                (
                    ...
                    f"non-WEIRD: {significance_stars(normal_corr['p'])}"
                ),
            ]
            ax.text(
                0.02,
        ...
        ax.set_title(titles[outcome], fontsize=18)
        ax.set_xlabel("Sentiment mean", fontsize=15)
    ...
    fig.suptitle("Sentiment vs Trust-Change Outcomes", fontsize=24)
    fig.savefig(output_path, dpi=250)
    plt.close(fig)
...
def main() -> None:
    ...
    sentiment_plot_path = FIGURES_DIR / "sentiment_by_group_condition.png"
    scatter_plot_path = FIGURES_DIR / "sentiment_vs_trust_changes.png"
