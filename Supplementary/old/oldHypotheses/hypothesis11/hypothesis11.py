...
PLOT_OUTPUT_PATH = OUTPUT_DIR / "hypothesis11.png"
REPORT_OUTPUT_PATH = OUTPUT_DIR / "hypothesis11.txt"
ALPHA = 0.05
...
def draw_group_subplot(
    ...
    interpretation = "No significant WEIRD vs NORMAL post-trust difference" if float(test_result["p"]) >= ALPHA else "Significant WEIRD vs NORMAL post-trust difference"
    summary = (
...
def write_report(
    ...
    lines.append("\\subsubsection{Hypothesis 11 Results}")
    lines.append(
    ...
    lines.append("11.1 WEIRD vs NORMAL difference in post-experiment overall trust:")
    lines.append(
        "Overall post-trust group contrast: "
    ...
    if float(overall_test["p"]) >= ALPHA:
        lines.append("Interpretation: no significant WEIRD vs NORMAL difference in post-experiment overall trust.")
    else:
        lines.append("Interpretation: significant WEIRD vs NORMAL difference in post-experiment overall trust.")

    lines.append("")
    lines.append("11.2 WEIRD vs NORMAL difference in post-experiment analytical trust:")
    lines.append(
        "Analytical post-trust group contrast: "
    ...
    if float(analytical_test["p"]) >= ALPHA:
        lines.append("Interpretation: no significant WEIRD vs NORMAL difference in post-experiment analytical trust.")
    else:
        lines.append("Interpretation: significant WEIRD vs NORMAL difference in post-experiment analytical trust.")

    lines.append("")
    lines.append("11.3 WEIRD vs NORMAL difference in post-experiment emotional trust:")
    lines.append(
        "Emotional post-trust group contrast: "
    ...
    if float(emotional_test["p"]) >= ALPHA:
        lines.append("Interpretation: no significant WEIRD vs NORMAL difference in post-experiment emotional trust.")
    else:
        lines.append("Interpretation: significant WEIRD vs NORMAL difference in post-experiment emotional trust.")
...
def main() -> None:
    ...
    overall_test = group_welch_test(analysis, "overall_post")
    analytical_test = group_welch_test(analysis, "analytical_post")
    emotional_test = group_welch_test(analysis, "emotional_post")
    ...
    draw_group_subplot(
        axes[0],
        overall_test,
        title="11.1 Post-Experiment Overall Trust (WEIRD vs NORMAL)",
        ylabel="Overall Post-Trust Score",
    )
    draw_group_subplot(
        axes[1],
        analytical_test,
        title="11.2 Post-Experiment Analytical Trust (WEIRD vs NORMAL)",
        ylabel="Analytical Post-Trust Score",
    )
    draw_group_subplot(
        axes[2],
        emotional_test,
        title="11.3 Post-Experiment Emotional Trust (WEIRD vs NORMAL)",
        ylabel="Emotional Post-Trust Score",
    )
    ...
    fig.suptitle("Hypothesis 11: WEIRD vs NORMAL Differences in Post-Experiment Trust", fontsize=22)
