...
if "Ethnicity" not in combined.columns:
	raise ValueError("Expected 'Ethnicity' column in dataset.")

TARGET_TOTAL_PER_CONDITION = 200
PRIORITY_ETHNICITIES = [
	"White",
	"Asian",
	"Black",
	"Mixed",
...
normalized = combined.assign(
	Ethnicity=combined["Ethnicity"].fillna("Missing").astype(str).str.strip().replace("", "Missing")
)

counts = (
	normalized
	.groupby(["Condition", "Ethnicity"], dropna=False)
	.size()
...
# Build one shared target ethnicity mix.
# Priority ethnicities are forced to the same target in both conditions.
pooled = counts.groupby("Ethnicity", as_index=False)["count"].sum()
pooled = pooled.rename(columns={"count": "pooled_count"})
pooled_total = int(pooled["pooled_count"].sum())

if pooled_total == 0:
	raise ValueError("No ethnicity counts found to compute balance targets.")

num_ethnicities = len(pooled)
if num_ethnicities == 0:
	raise ValueError("No ethnicities found to compute balance targets.")
...
for ethnicity in PRIORITY_ETHNICITIES:
	if ethnicity in count_wide.index:
		priority_targets[ethnicity] = int(count_wide.loc[ethnicity, ordered_conditions].max())
...
if remaining_total < 0:
	raise ValueError(
		"Priority ethnicity targets exceed target total per condition. "
		f"Reserved={reserved_target_total}, target={TARGET_TOTAL_PER_CONDITION}."
	)

non_priority = pooled[~pooled["Ethnicity"].isin(priority_targets.keys())].copy()
non_priority_total = int(non_priority["pooled_count"].sum())

if len(non_priority) == 0 and remaining_total > 0:
	raise ValueError("No non-priority ethnicities available for remaining target allocation.")
...
if remaining_total > 0:
	...
	non_priority = non_priority.sort_values(["fraction", "Ethnicity"], ascending=[False, True]).reset_index(drop=True)
...
priority_df = pd.DataFrame(
	{"Ethnicity": list(priority_targets.keys()), "desired_target_count": list(priority_targets.values())}
)

targets = pd.concat(
	[priority_df, non_priority[["Ethnicity", "desired_target_count"]]],
	ignore_index=True,
)

# Ensure every ethnicity appears for both conditions (missing counts become 0).
all_pairs = pd.MultiIndex.from_product(
	[ordered_conditions, targets["Ethnicity"].tolist()],
	names=["Condition", "Ethnicity"],
).to_frame(index=False)

counts = all_pairs.merge(counts, on=["Condition", "Ethnicity"], how="left")
counts["count"] = counts["count"].fillna(0).astype(int)
counts = counts.merge(targets, on="Ethnicity", how="left")
...
counts["is_priority"] = counts["Ethnicity"].isin(PRIORITY_ETHNICITIES)
...
for condition in ordered_conditions:
	...
	if non_priority_deficit_total > 0:
		...
		if len(non_priority_rows) > 0:
			...
			if remainder > 0:
				ordered_idx = non_priority_rows.sort_values("Ethnicity").index
...
counts = counts.sort_values(["Condition", "target_count", "Ethnicity"], ascending=[True, False, True])

print("Ethnicity counts by condition:")
...
counts.to_csv("Balanced Ethnicities.csv", index=False)
