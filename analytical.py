...
# These items are negatively keyed for trust; flip sign so positive delta means higher trust.
...
for pre_col, post_col in ANALYTICAL_PAIRS:
    ...
    pre = _to_likert_numeric(combined[pre_col])
    post = _to_likert_numeric(combined[post_col])
    delta = post - pre
...
if not participant_deltas:
    raise ValueError("No analytical trust pre/post column pairs found in dataset.")


delta_df = pd.concat(participant_deltas, axis=1)
combined["Analytical Trust Impact Delta"] = delta_df.mean(axis=1, skipna=True)

analysis_df = combined[["Condition", "Analytical Trust Impact Delta"]].dropna()
...
for condition in ordered_conditions:
    values = analysis_df.loc[
        analysis_df["Condition"] == condition, "Analytical Trust Impact Delta"
    ].to_numpy()
...
print("Overall analytical trust impact by condition (Post - Pre):")
print(summary_df.to_string(index=False))

cond_a_vals = analysis_df.loc[
    analysis_df["Condition"] == ordered_conditions[0], "Analytical Trust Impact Delta"
].to_numpy()
cond_b_vals = analysis_df.loc[
    analysis_df["Condition"] == ordered_conditions[1], "Analytical Trust Impact Delta"
].to_numpy()
if len(cond_a_vals) >= 2 and len(cond_b_vals) >= 2:
    ...
    print(
        f"\nBetween-condition test on overall analytical trust impact: "
...
plt.title("Overall Analytical Trust Impact by Condition")
plt.xlabel("Condition")
plt.ylabel("Mean Analytical Trust Impact (Post - Pre)")
...
plt.tight_layout()
plot_path = out_dir / "overall_analytical_trust_impact_by_condition.png"
plt.savefig(plot_path, dpi=300, bbox_inches="tight")
plt.close()

summary_path = out_dir / "overall_analytical_trust_impact_by_condition.csv"
summary_df.to_csv(summary_path, index=False)

print(f"\nSaved analytical trust summary to: {summary_path}")
print(f"Saved analytical trust impact plot to: {plot_path}")
