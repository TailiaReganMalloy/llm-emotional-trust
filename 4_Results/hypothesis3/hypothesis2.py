...
TXT_PATH = OUTPUT_DIR / "hypothesis2.txt"
PNG_PATH = OUTPUT_DIR / "hypothesis2.png"
ALPHA = 0.05
...
def draw_type_subplot(ax: plt.Axes, title: str, stats: dict[str, float]) -> None:
    ...
    ax.set_xticklabels(labels)
    ax.set_title(title, pad=14)
    ax.set_ylabel("Standardized Change")
...
def main() -> None:
    ...
    analytical_stats = paired_pre_post(weird["Total Analytical Trust"], weird["Total Analytical Trust Post"])
    emotional_stats = paired_pre_post(weird["Total Emotional Trust"], weird["Total Emotional Trust Post"])
    type_stats = compare_trust_type(weird)

    fig, axes = plt.subplots(1, 4, figsize=(22, 5.8), constrained_layout=True)
    draw_pre_post_subplot(axes[0], "2.1 Overall Pre vs Post", overall_stats)
    draw_pre_post_subplot(axes[1], "2.2 Analytical Pre vs Post", analytical_stats)
    draw_pre_post_subplot(axes[2], "2.3 Emotional Pre vs Post", emotional_stats)
    draw_type_subplot(axes[3], "2.4 Emotional vs Analytical Difference", type_stats)
    ...
    fig.suptitle("Hypothesis 2: WEIRD Trust Change Pattern", fontsize=24)
