...
PNG_PATH = OUTPUT_DIR / "hypothesis5.png"
ALPHA = 0.05
...
def draw_left_subplot(
    ...
    ax.set_xticks(x)
    ax.set_xticklabels(["WEIRD", "NORMAL", "WEIRD", "NORMAL"], fontsize=11)
    ax.text(0.5, -0.14, "Emotional z", transform=ax.get_xaxis_transform(), ha="center", va="top", fontsize=11)
    ax.text(3.5, -0.14, "Analytical z", transform=ax.get_xaxis_transform(), ha="center", va="top", fontsize=11)
    ax.set_ylabel("Standardized Change")
    ax.set_title("5.1 and 5.2 Group Contrasts (Emotional z and Analytical z)")
...
def draw_right_subplot(ax: plt.Axes, overall_test: dict[str, float]) -> None:
    ...
    ax.axhline(0.0, color="#4a4a4a", linewidth=1.5, zorder=1)
    ax.set_xticks(x)
    ax.set_xticklabels(["WEIRD", "NORMAL"], fontsize=11)
    ax.set_ylabel("Standardized Change")
    ax.set_title("5.3 Group Contrast (Overall z)")
    ax.set_ylim(y_min - 0.22 * y_span, y_max + 0.40 * y_span)
...
def main() -> None:
    ...
    fig.suptitle("Hypothesis 5: WEIRD vs NORMAL Standardized Trust-Change Contrasts", fontsize=22)
